...
class DataLoader:
    ...
    def load_data(data_config, prefix="blond"):
        ...
        train_generator = train_datagen.flow_from_directory(
            ...
            target_size=(
                int(data_config.data.IMG_HEIGHT),
                int(data_config.data.IMG_WIDTH),
            ),
...
def generate_df(data_config, attr):
    ...
    df_train = df_par_attr[
        (df_par_attr["partition"] == 0) & (df_par_attr[attr] == 0)
    ].sample(int(int(data_config.data.TRAINING_SAMPLES) / 2))

    df_train = pd.concat(
        [
            df_train,
            df_par_attr[
                (df_par_attr["partition"] == 0) & (df_par_attr[attr] == 1)
            ].sample(int(int(data_config.data.TRAINING_SAMPLES) / 2)),
        ]
    )

    df_val = df_par_attr[
        (df_par_attr["partition"] == 1) & (df_par_attr[attr] == 0)
    ].sample(
        int(int(data_config.data.VALIDATION_SAMPLES) / 2)
    ...
    df_val = pd.concat(
        [
            df_val,
            df_par_attr[
                (df_par_attr["partition"] == 1) & (df_par_attr[attr] == 1)
            ].sample(int(int(data_config.data.VALIDATION_SAMPLES) / 2)),
        ]
    )  # file names for validation

    df_test = df_par_attr[
        (df_par_attr["partition"] == 2) & (df_par_attr[attr] == 0)
    ].sample(int(int(data_config.data.TEST_SAMPLES) / 2))
    # file names for test
    df_test = pd.concat(
        [
            df_test,
            df_par_attr[
                (df_par_attr["partition"] == 2) & (df_par_attr[attr] == 1)
            ].sample(int(int(data_config.data.TEST_SAMPLES) / 2)),
        ]
    )  # file names for test
...
def copy_images(folder_prefix, attribute, df_images, df_type):
    ...
    for i, j in df_images.iterrows():
        if j[attribute] == 0:
            shutil.copy(
                "data/CelebA/img_align_celeba/img_align_celeba/" + i,
                "data/celeba-dataset/{}-{}/0/{}".format(folder_prefix, df_type, i),
            )
        if j[attribute] == 1:
            shutil.copy(
                "data/CelebA/img_align_celeba/img_align_celeba/" + i,
                "data/celeba-dataset/{}-{}/1/{}".format(folder_prefix, df_type, i),
            )
